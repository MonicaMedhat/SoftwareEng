    ...
    $con->set_charset("utf8");
           ...
           echo "<select name='CaseT'>"; // start a table tag in the HTML
               ...
               while($row = mysqli_fetch_array($result)){  ?>  //Creates a loop to loop through results
                   ...
                   <option value="<?php echo $value['ID'];?>"><?php echo $row['Name']; ?></option>
...
<option value="pm">PM</option>
           ...
           <form id="form1">
            ...
            <br>
          <?php
               $sql="SELECT FullName FROM User WHERE UserType_ID=2";
	       $result=$db_obj->executesql($sql);
           echo "<select name='SelectLawyer'>"; // start a table tag in the HTML

               while($row = mysqli_fetch_array($result)){   //Creates a loop to loop through results
                   echo "<option>" . $row['Name'] . "</option>";  //$row['index'] the index here is a field name
               }
           ...
           <h4 >ابحث عن الموكل هنا</h4>
               <?php
               $sql="SELECT FullName FROM User WHERE UserType_ID=4";
	       $result=$db_obj->executesql($sql);
